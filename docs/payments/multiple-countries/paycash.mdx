...
Paycash is a payment method designed to offer flexibility by accommodating both traditional cash payments at physical locations and modern digital transactions through banking apps and wallets.

It serves as a link between the physical and digital payment worlds, catering to a broader range of consumer preferences and needs. A key feature of the 

Paycash system is the **issuance of a barcode** to the user, which serves as the token for completing the payment through the various supported methods.
...
sequenceDiagram
    ...
    participant epag
    participant PaycashProvider as Paycash Provider

    Payer ->> Merchant: Paycash Checkout
    Merchant ->> epag: Paycash Payment Request
    epag ->> PaycashProvider: Generate Paycash Reference
    PaycashProvider ->> epag: Paycash Reference
    epag ->> Merchant: Paycash Reference
    Merchant ->> Payer: Share Paycash Reference
    PaycashProvider -->> Payer: Email With Payment Instructions
    Payer -->> PaycashProvider: Payment at Paycash Location
    PaycashProvider -->> epag: Payment Notification
